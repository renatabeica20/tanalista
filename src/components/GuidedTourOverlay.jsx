...
export default function GuidedTourOverlay({
  ...
  // Busca o elemento: primeiro por data-tour-step exato,
  // depois por data-tour-also (atributo que lista IDs alternativos separados por espaço)
  const findEl = useCallback(() => {
    ...
    if (el) return el;
    // 2. Atributo data-tour-also (ex: "create_item_voice create_item_paste")
  ...
  useEffect(() => {
    ...
    if (!attempt()) {
      ...
      // Re-mede depois do scroll terminar
      timers.push(setTimeout(() => {
        if (cancelled) return;
  ...
  // clip-path com buraco: exterior horário, buraco anti-horário
  const W = typeof window !== "undefined" ? window.innerWidth : 430;
  const H = typeof window !== "undefined" ? window.innerHeight : 900;
  const r = targetRect;
  const backdropClipPath = r
    ? `polygon(
        0px 0px, ${W}px 0px, ${W}px ${H}px, 0px ${H}px, 0px 0px,
        ${r.x}px ${r.y}px, ${r.x}px ${r.y + r.h}px,
        ${r.x + r.w}px ${r.y + r.h}px, ${r.x + r.w}px ${r.y}px,
        ${r.x}px ${r.y}px
      )`
    : undefined;

  return (
    <div style={{ position:"fixed", inset:0, zIndex:620, pointerEvents:"none", animation:"tnl-tour-fade 220ms ease-out" }}>
      <style>{`
        @keyframes tnl-tour-fade { from{opacity:0} to{opacity:1} }
        @keyframes tnl-tour-pop  { from{transform:translateY(16px) scale(0.96);opacity:0} to{transform:translateY(0) scale(1);opacity:1} }
        ...
        @keyframes tnl-tour-ring {
          0%,100%{ box-shadow:0 0 0 3px rgba(255,255,255,0.9), 0 0 0 6px rgba(124,58,237,0.7); }
          50%    { box-shadow:0 0 0 5px rgba(255,255,255,1),   0 0 0 10px rgba(124,58,237,0.9); }
        }
      `}</style>

      {/* Backdrop escuro com recorte no elemento alvo */}
      <div style={{
      ...
      <div style={{
        ...
        <div style={{position:"relative",height:8,background:"linear-gradient(180deg,#F1EEFB 0%,#EDE9FE 100%)",borderRadius:999,overflow:"hidden",margin:"10px 0 18px",boxShadow:"inset 0 1px 2px rgba(76,29,149,0.10)"}}>
          <div style={{position:"relative",height:"100%",width:`${progress}%`,background:"linear-gradient(90deg,#4C1D95 0%,#7C3AED 50%,#9F67FA 100%)",borderRadius:999,transition:"width 500ms cubic-bezier(0.22,1,0.36,1)",boxShadow:"0 0 12px rgba(124,58,237,0.5),inset 0 1px 0 rgba(255,255,255,0.35)",overflow:"hidden"}}>
