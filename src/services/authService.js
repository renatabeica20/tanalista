...
export async function verifyOrCreateUserPin(
  name,
  pin,
  pinConfirm = ""
) {
  ...
  if (!isValidPin(safePin)) {
    return {
      ok: false,
      message: "Informe um PIN de 4 a 6 dígitos.",
    };
  }
  ...
  if (existing?.data?.pinHash) {
    if (existing.data.pinHash !== hash) {
      return {
        ok: false,
        message: "PIN incorreto para este usuário.",
      };
    }
  ...
  if (!safeConfirm) {
    return {
      ok: false,
      message: "Primeiro acesso: confirme o PIN para criar seu cadastro.",
    };
  }
